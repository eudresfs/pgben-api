...
async function verifySeeds() {
  try {
    ...
    unidades.forEach((u: any) => {
      console.log(
        `- ${u.nome} (${u.codigo}): tipo=${u.tipo}, status=${u.status}`,
      );
    });
    ...
    setores.forEach((s: any) => {
      console.log(
        `- ${s.nome} (${s.sigla}): unidade_id=${s.unidade_id}, status=${s.status}`,
      );
    });
    ...
    usuarios.forEach((u: any) => {
      console.log(
        `- ${u.nome} (${u.email}): role_id=${u.role_id}, status=${u.status}`,
      );
    });
    ...
    tiposBeneficio.forEach((tb: any) => {
      console.log(
        `- ${tb.nome} (${tb.codigo}): periodicidade=${tb.periodicidade}, ativo=${tb.ativo}`,
      );
    });
    ...
    permissoes.forEach((p: any) => {
      console.log(
        `- ${p.nome}: modulo=${p.modulo}, acao=${p.acao}, ativo=${p.ativo}`,
      );
    });
