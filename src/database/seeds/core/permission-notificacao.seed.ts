...
export class PermissionNotificacaoSeed {
  ...
  static async run(dataSource: DataSource): Promise<void> {
    this.logger.log(
      'Iniciando seed de permissões do módulo de notificações...',
    );
    ...
    try {
      ...
      this.logger.log(
        'Seed de permissões do módulo de notificações concluído com sucesso!',
      );
    } catch (error) {
      this.logger.error(
        `Erro ao executar seed de permissões de notificações: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  private static async createGranularPermissions(
    ...
    this.logger.log(
      'Criando permissões granulares do módulo de notificações...',
    );
    ...
    this.logger.log(
      `${permissions.length} permissões granulares do módulo de notificações processadas.`,
    );
  }
    ...
    try {
      ...
      const result = await dataSource.query(
        `INSERT INTO permissao (nome, descricao, modulo, acao, status) 
         VALUES ($1, $2, $3, $4, $5) 
         RETURNING id`,
        [nome, descricao, 'notificacao', acao, Status.ATIVO],
      );
