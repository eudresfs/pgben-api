...
export class PermissionJudicialSeed {
  ...
  static async run(dataSource: DataSource): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        'Seed de permissões do módulo judicial concluído com sucesso!',
      );
    } catch (error) {
      this.logger.error(
        `Erro ao executar seed de permissões judicial: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  private static async createGranularPermissions(
    ...
    this.logger.log(
      `${permissions.length} permissões granulares do módulo judicial processadas.`,
    );
  }
