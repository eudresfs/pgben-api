...
export class PermissionMetricasSeed {
  ...
  static async run(dataSource: DataSource): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        'Seed de permissões do módulo de métricas concluído com sucesso!',
      );
    } catch (error) {
      this.logger.error(
        `Erro ao executar seed de permissões de métricas: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  private static async createGranularPermissions(
    ...
    this.logger.log(
      `${permissions.length} permissões granulares do módulo de métricas processadas.`,
    );
  }
