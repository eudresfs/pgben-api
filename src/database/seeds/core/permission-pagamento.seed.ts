...
export class PermissionPagamentoSeed {
  ...
  static async run(dataSource: DataSource): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        'Seed de permissões do módulo de pagamentos concluído com sucesso!',
      );
    } catch (error) {
      this.logger.error(
        `Erro ao executar seed de permissões de pagamentos: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  private static async createGranularPermissions(
    ...
    this.logger.log(
      `${permissions.length} permissões granulares do módulo de pagamentos processadas.`,
    );
  }
