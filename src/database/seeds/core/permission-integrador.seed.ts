...
export class PermissionIntegradorSeed {
  ...
  static async run(dataSource: DataSource): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        'Seed de permissões do módulo de integrador concluído com sucesso!',
      );
    } catch (error) {
      this.logger.error(
        `Erro ao executar seed de permissões de integrador: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  private static async createGranularPermissions(
    ...
    this.logger.log(
      `${permissions.length} permissões granulares do módulo de integrador processadas.`,
    );
  }
