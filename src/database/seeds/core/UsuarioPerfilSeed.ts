...
export class UsuarioPerfilSeed extends BaseSeed {
  ...
  protected async verificarEstruturaDasTabelas(
  ...
  private async criarPerfis(dataSource: DataSource): Promise<void> {
    ...
    const { colunas, statusColumnName } = await this.obterInformacoesTabela(
      dataSource,
      'role',
    );
      ...
      await this.executarComTratamento(
        ...
        async () => {
          ...
          if (roleExistente.length === 0) {
            ...
            const { query, params } = this.construirQueryInsercao(
              'role',
              colunasInsercao,
              valoresInsercao,
            );
  ...
  private async criarUsuarioAdmin(dataSource: DataSource): Promise<void> {
    ...
    if (roleAdmin.length === 0) {
      throw new Error(
        'Perfil Administrador não encontrado. Verifique a execução do seed de perfis.',
      );
    }
