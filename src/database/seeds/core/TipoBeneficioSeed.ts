  ...
  public static async run(dataSource: DataSource): Promise<void> {
    ...
    try {
      ...
      if (tableInfo.length === 0) {
        throw new Error(
          'Tabela tipo_beneficio não encontrada no banco de dados',
        );
      }

      const columnNames = tableInfo.map((col) => col.column_name);
      console.log(
        `Colunas encontradas na tabela tipo_beneficio: ${columnNames.join(', ')}`,
      );
      ...
      for (const tipo of tiposBeneficio) {
        try {
          ...
          if (existingTipo.length === 0) {
            ...
            const colunasExistentes: string[] = colunas.filter(
              (col) => col === statusColumnName || columnNames.includes(col),
            );

            // Criar placeholders para os valores ($1, $2, etc.)
            const placeholders: string[] = colunasExistentes.map(
              (_, index) => `$${index + 1}`,
            );
          ...
          console.error(
            `Erro ao processar tipo de benefício ${tipo.nome}: ${error.message}`,
          );
      ...
      console.error(
        `Erro global no seed de tipos de benefícios: ${error.message}`,
      );
      throw error;
    }
  }
