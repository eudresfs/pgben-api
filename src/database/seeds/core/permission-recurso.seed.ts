...
export class PermissionRecursoSeed {
  ...
  static async run(dataSource: DataSource): Promise<void> {
    ...
    try {
      ...
      this.logger.log('Seed de permissões do módulo de recursos concluído com sucesso!');
    } catch (error) {
      this.logger.error(`Erro ao executar seed de permissões de recursos: ${error.message}`);
      throw error;
    }
  }
  ...
  private static async createGranularPermissions(
    ...
    this.logger.log(`${permissions.length} permissões granulares do módulo de recursos processadas.`);
  }
    ...
    try {
      ...
      const result = await dataSource.query(
        `INSERT INTO permissao (nome, descricao, modulo, acao, ativo) 
         VALUES ($1, $2, $3, $4, $5) 
         RETURNING id`,
        [nome, descricao, 'recurso', acao, true]
      );
