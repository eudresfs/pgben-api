...
export class PermissionOcorrenciaSeed {
  ...
  static async run(dataSource: DataSource): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        'Seed de permissões do módulo de ocorrências concluído com sucesso!',
      );
    } catch (error) {
      this.logger.error(
        `Erro ao executar seed de permissões de ocorrências: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  private static async createGranularPermissions(
    ...
    this.logger.log(
      'Criando permissões granulares do módulo de ocorrências...',
    );
    ...
    this.logger.log(
      `${permissions.length} permissões granulares do módulo de ocorrências processadas.`,
    );
  }
