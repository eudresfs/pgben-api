...
export class PermissionCidadaoSeed {
  ...
  public static async run(dataSource: DataSource): Promise<void> {
    ...
    try {
      ...
      console.log(
        'Seed de permissões do módulo de cidadão concluído com sucesso!',
      );
    } catch (error) {
      console.error(
        `Erro ao executar seed de permissões do módulo de cidadão: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  private static async createPermissionScope(
    ...
    if (existingScope) {
      console.log(
        `Escopo para permissão '${permissaoId}' já existe, atualizando...`,
      );
