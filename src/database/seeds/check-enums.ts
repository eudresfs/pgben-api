  ...
  try {
    ...
    try {
      ...
      if (periodicidadeEnum && periodicidadeEnum.length > 0) {
        ...
        console.log(
          'Não foi possível obter os valores de periodicidade_beneficio',
        );
      }
    } catch (error) {
      console.error(
        'Erro ao verificar enum periodicidade_beneficio:',
        error.message,
      );
    }
    ...
    for (const table of tables) {
      ...
      try {
        ...
        columns.forEach((col: any) => {
          console.log(
            `- ${col.column_name}: ${col.data_type}${col.udt_name !== col.data_type ? ` (${col.udt_name})` : ''}, nullable=${col.is_nullable}`,
          );
        });
              ...
              con.constraint_type === 'p'
                ...
                : con.constraint_type === 'f'
                  ...
                  : con.constraint_type === 'u'
                    ? 'UNIQUE'
                    : con.constraint_type === 'c'
                      ? 'CHECK'
                      : con.constraint_type;
            console.log(
              `- ${con.constraint_name} (${type}): ${con.constraint_definition}`,
            );
          });
        }
      } catch (error) {
