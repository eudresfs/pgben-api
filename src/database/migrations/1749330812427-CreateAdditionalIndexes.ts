 ...
 * @author Equipe PGBen
 */
export class CreateAdditionalIndexes1749330812427
  implements MigrationInterface
{
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP INDEX IF EXISTS idx_solicitacao_data_abertura`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS idx_solicitacao_unidade_status`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS idx_comprovante_pagamento_id`,
    );
    ...
    await queryRunner.query(
      `DROP INDEX IF EXISTS idx_info_bancaria_banco_agencia_conta`,
    );
  }
}
