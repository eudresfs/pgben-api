 ...
 */
export class RemoveRedundantCidadaoColumns1750700200000
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (!contatoTableExists || !enderecoTableExists) {
      throw new Error(
        'As tabelas contato e endereco devem existir antes de remover as colunas redundantes do cidadao',
      );
    }

    // Verificar se há dados nas novas tabelas
    const contatoCount = await queryRunner.query(
      'SELECT COUNT(*) as count FROM contato',
    );
    const enderecoCount = await queryRunner.query(
      'SELECT COUNT(*) as count FROM endereco',
    );
    ...
    console.log(
      'Migration RemoveRedundantCidadaoColumns concluída com sucesso!',
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    console.log(
      'Colunas redundantes recriadas. ATENÇÃO: Os dados não foram restaurados!',
    );
  }
}
