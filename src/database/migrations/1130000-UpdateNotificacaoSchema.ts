...
export class UpdateNotificacaoSchema1685555123456 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (tableExists[0].exists) {
      ...
      const createdAtExists = await queryRunner.query(`
        SELECT EXISTS (
          SELECT 1 FROM information_schema.columns 
          WHERE table_name = 'notificacoes_sistema' AND column_name = 'created_at'
        );
      `);
      
      if (createdAtExists[0].exists) {
        console.log('Renomeando coluna created_at para criado_em...');
        await queryRunner.query(`
          ALTER TABLE "notificacoes_sistema" 
      ...
      const updatedAtExists = await queryRunner.query(`
        SELECT EXISTS (
          SELECT 1 FROM information_schema.columns 
          WHERE table_name = 'notificacoes_sistema' AND column_name = 'updated_at'
        );
      `);
      
      if (updatedAtExists[0].exists) {
        console.log('Renomeando coluna updated_at para atualizado_em...');
        await queryRunner.query(`
          ALTER TABLE "notificacoes_sistema" 
      ...
      console.log('Tabela notificacoes_sistema não existe, pulando atualização.');
    }
    
    console.log('Migration 1130000-UpdateNotificacaoSchema executada com sucesso.');
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    console.log('Migration 1130000-UpdateNotificacaoSchema revertida com sucesso.');
  }
}
