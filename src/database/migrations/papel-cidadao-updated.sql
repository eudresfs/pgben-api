...
-- Primeiro, remover a view que depende da coluna tipo_cidadao
...
-- Recriar a view vw_beneficiarios_ativos com a nova estrutura
CREATE OR REPLACE VIEW vw_beneficiarios_ativos AS
SELECT c.*
FROM cidadao c
JOIN papel_cidadao pc ON c.id = pc.cidadao_id
WHERE pc.tipo_papel = 'beneficiario' AND pc.ativo = true AND c.ativo = true;
