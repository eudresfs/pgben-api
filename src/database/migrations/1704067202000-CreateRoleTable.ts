 ...
 */
export class CreateRoleTable1704067202000 implements MigrationInterface {
  name = 'CreateRoleTable1704067202000';
    ...
    if (checkEnum && checkEnum.length > 0) {
      ...
      if (checkDependencies && checkDependencies.length > 0) {
        ...
        checkDependencies.forEach((dep) => {
          console.log(
            `- Tabela: ${dep.table_name}, Coluna: ${dep.column_name}`,
          );
        });
