  ...
  try {
    ...
    if (colunas.length === 0) {
      ...
      colunas.forEach((coluna: any) => {
        console.log(
          `- ${coluna.column_name} (${coluna.data_type}, ${coluna.is_nullable === 'YES' ? 'nullable' : 'not nullable'})`,
        );
      });
    }
    ...
    const setorEntity = entidades.find(
      (entity) => entity.tableName === 'setor',
    );

    if (!setorEntity) {
      console.log(
        'A entidade "Setor" não foi encontrada nas entidades carregadas.',
      );
