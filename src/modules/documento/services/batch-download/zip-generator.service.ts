...
export class ZipGeneratorService {
  ...
  async gerarZipStream(
    ...
    this.logger.log(`Iniciando geração de ZIP para ${documentos.length} documentos (Job: ${jobId})`);
    ...
    this.processarArquivosAsync(archive, zipStructure.files, jobId)
      ...
      .catch((error) => {
        this.logger.error(`Erro ao processar arquivos para job ${jobId}:`, error);
  ...
  private async processarArquivosAsync(
    ...
    for (let i = 0; i < files.length; i += batchSize) {
      ...
      await Promise.allSettled(
        batch.map(fileInfo => this.adicionarArquivoAoZip(archive, fileInfo, jobId))
      );
      
      // Log de progresso
      const progresso = Math.min(i + batchSize, files.length);
      this.logger.debug(`Processados ${progresso}/${files.length} arquivos (Job: ${jobId})`);
    }
  }
  ...
  private async adicionarArquivoAoZip(
    ...
    try {
      ...
      const arquivoBuffer = await storageProvider.obterArquivo(fileInfo.documento.caminho);
  ...
  private configurarEventHandlers(
    ...
    archive.on('progress', (progress) => {
      this.logger.debug(`Progresso ZIP (Job: ${jobId}): ${progress.entries.processed}/${progress.entries.total}`);
    });
   ...
   */
  private agruparDocumentosPorTipo(documentos: Documento[]): Record<string, Documento[]> {
  ...
  private gerarIndiceDocumentos(files: ZipFileInfo[], jobId: string): string {
    ...
    const porTipo = files.reduce((grupos, file) => {
    ...
    for (const [tipo, arquivos] of Object.entries(porTipo)) {
      ...
      arquivos.forEach((file, index) => {
        const tamanhoFormatado = file.tamanho ? this.formatarTamanho(file.tamanho) : 'N/A';
        lines.push(`   ${index + 1}. ${file.documento.nome_arquivo} (${tamanhoFormatado})`);
      });
