...
export class DocumentoUrlService {
  ...
  async generatePublicUrl(documentoId: string): Promise<string> {
    ...
    try {
      this.logger.debug(
        'Gerando URL pública para documento',
  ...
  async generatePrivateUrl(
    documentoId: string,
    ttlHours: number = this.defaultTtlHours,
  ): Promise<string> {
    const startTime = Date.now();

    try {
      this.logger.debug(
        'Gerando URL privada para documento',
        DocumentoUrlService.name,
        {
          documentoId,
          ttlHours,
      ...
      this.logger.info(
        'URL privada gerada com sucesso',
        DocumentoUrlService.name,
        {
          documentoId,
  ...
  async validatePrivateAccess(hash: string): Promise<{ documentoId: string }> {
    ...
    try {
      this.logger.debug(
        'Validando acesso via URL privada',
        DocumentoUrlService.name,
        {
          hash: hash.substring(0, 8) + '...', // Log apenas parte do hash por segurança
        },
      ...
      if (!cachedData) {
        this.logger.warn(
          'URL privada não encontrada ou expirada',
          DocumentoUrlService.name,
          {
      ...
      this.logger.info(
        'Acesso via URL privada validado com sucesso',
        DocumentoUrlService.name,
        {
  ...
  async listActivePrivateUrls(documentoId: string): Promise<string[]> {
    try {
      ...
      this.logger.warn(
        'Método listActivePrivateUrls não implementado - CacheService não possui método keys',
        DocumentoUrlService.name,
        {
          documentoId,
        },
