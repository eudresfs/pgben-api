...
export class AuditoriaExportacaoService {
  ...
  async exportarLogs(
    ...
    if (opcoes.formato !== FormatoExportacao.JSON && opcoes.formato !== FormatoExportacao.CSV) {
  ...
  private processarCampos(
    ...
    return logs.map(log => {
      ...
      const camposParaIncluir = campos?.length 
        ? campos 
        : Object.keys(log);
  ...
  private gerarNomeArquivo(opcoes: OpcoesExportacao): string {
    ...
    const timestamp = dataAtual.toISOString().replace(/[:.]/g, '-').replace('T', '_').split('Z')[0];
   ...
   * @private
   */
  private sanitizarDadosParaExportacao(dados: unknown[]): Record<string, unknown>[] {
    ...
    try {
      return dados.map((item) => {
        ...
        Object.entries(item as Record<string, unknown>).forEach(([chave, valor]) => {
          // Preservar estrutura para campos específicos que contêm objetos ou arrays
          if (camposEstruturados.includes(chave) && valor !== null && typeof valor === 'object') {
