...
import { ProcessoJudicial, StatusProcessoJudicial } from '../entities/processo-judicial.entity';
...
export class ProcessoJudicialService {
   ...
   */
  async create(data: Partial<ProcessoJudicial>, usuarioId: string): Promise<ProcessoJudicial> {
    ...
    if (existente) {
      throw new BadRequestException(`Já existe um processo judicial com o número: ${data.numero_processo}`);
    }
  ...
  async findById(id: string): Promise<ProcessoJudicial> {
    ...
    if (!processo) {
      throw new NotFoundException(`Processo judicial com ID ${id} não encontrado`);
    }
   ...
   * @throws NotFoundException se o processo não for encontrado
   */
  async findByNumeroProcesso(numeroProcesso: string): Promise<ProcessoJudicial> {
    const processo = await this.processoJudicialRepository.findOne({
    ...
    if (!processo) {
      throw new NotFoundException(`Processo judicial número ${numeroProcesso} não encontrado`);
    }
   ...
   * @throws NotFoundException se o processo não for encontrado
   */
  async update(id: string, data: Partial<ProcessoJudicial>, usuarioId: string): Promise<ProcessoJudicial> {
    const processo = await this.findById(id);
    
    // Verificar se está tentando alterar o número do processo para um número já existente
    if (data.numero_processo && data.numero_processo !== processo.numero_processo) {
      const existente = await this.processoJudicialRepository.findOne({
        where: { numero_processo: data.numero_processo }
      });
   ...
   * @throws NotFoundException se o processo não for encontrado
   */
  async updateStatus(id: string, status: StatusProcessoJudicial, usuarioId: string): Promise<ProcessoJudicial> {
    const processo = await this.findById(id);
  ...
  async getEstatisticas(): Promise<{
    ...
    const porStatus = Object.values(StatusProcessoJudicial).reduce((acc, status) => {
      acc[status] = 0;
      return acc;
