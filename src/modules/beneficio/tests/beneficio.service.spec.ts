...
import { TipoBeneficio, Periodicidade } from '../entities/tipo-beneficio.entity';
import { RequisitoDocumento, TipoDocumento } from '../entities/requisito-documento.entity';
...
describe('BeneficioService', () => {
  ...
  describe('configurarFluxo', () => {
    it('deve configurar o fluxo de aprovação de um benefício', async () => {
      ...
      mockFluxoBeneficioRepository.save.mockResolvedValue([mockEtapa1, mockEtapa2]);
      ...
      expect(mockFluxoBeneficioRepository.save).toHaveBeenCalledWith([mockEtapa1, mockEtapa2]);
    });
    ...
    it('deve lançar BadRequestException quando o fluxo não contém etapas', async () => {
      ...
      await expect(service.configurarFluxo('1', configurarFluxoDto)).rejects.toThrow(BadRequestException);
      expect(mockTipoBeneficioRepository.findOne).toHaveBeenCalled();
