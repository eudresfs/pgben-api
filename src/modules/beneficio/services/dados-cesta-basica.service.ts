...
> {
  ...
  protected async validateCreateData(
    data: CreateDadosCestaBasicaDto,
  ): Promise<void> {
    ...
    try {
      ...
      if (!data.solicitacao_id?.trim()) {
        errorBuilder.add(
          'solicitacao_id',
          BENEFICIO_TECH_MESSAGES.CESTA_BASICA.SOLICITACAO_ID_REQUIRED,
        );
      }

      if (
        !data.quantidade_cestas_solicitadas ||
        data.quantidade_cestas_solicitadas <= 0
      ) {
        errorBuilder.add(
          'quantidade_cestas_solicitadas',
          BENEFICIO_TECH_MESSAGES.CESTA_BASICA.QUANTIDADE_CESTAS_REQUIRED,
        );
      ...
      if (!data.numero_pessoas_familia || data.numero_pessoas_familia <= 0) {
        errorBuilder.add(
          ...
          BENEFICIO_TECH_MESSAGES.CESTA_BASICA.NUMERO_PESSOAS_REQUIRED,
        );
      ...
      if (data.quantidade_cestas_solicitadas && data.numero_pessoas_familia) {
        const quantidadeRecomendada = this.calcularQuantidadeRecomendada(
          data.numero_pessoas_familia,
        );
        ...
        if (data.quantidade_cestas_solicitadas > quantidadeRecomendada) {
          if (!data.justificativa_quantidade?.trim()) {
            errorBuilder.add(
              ...
              BENEFICIO_TECH_MESSAGES.CESTA_BASICA.JUSTIFICATIVA_REQUIRED,
            );
          } else if (
            data.justificativa_quantidade.trim().length <
            rules.MIN_JUSTIFICATIVA
          ) {
            errorBuilder.add(
              'justificativa_quantidade',
              `Justificativa deve ter pelo menos ${rules.MIN_JUSTIFICATIVA} caracteres. Validação de tamanho falhou.`,
            );
          }
        }
      }
      ...
      if (
        ...
        data.tecnico_responsavel.trim().length === 0
      ) {
        errorBuilder.add(
          'tecnico_responsavel',
          'Campo tecnico_responsavel não pode estar vazio quando fornecido. Validação de conteúdo falhou.',
        );
      }

      // Validação de unidade solicitante (se fornecida)
      if (
        data.unidade_solicitante &&
        data.unidade_solicitante.trim().length === 0
      ) {
        errorBuilder.add(
          'unidade_solicitante',
          'Campo unidade_solicitante não pode estar vazio quando fornecido. Validação de conteúdo falhou.',
        );
      }
      ...
      throw new InternalServerErrorException(
        'Erro interno durante validação dos dados de cesta básica',
      );
    }
  }
    ...
    try {
      ...
      if (data.quantidade_cestas_solicitadas !== undefined) {
        if (data.quantidade_cestas_solicitadas <= 0) {
          errorBuilder.add(
            'quantidade_cestas_solicitadas',
            'Quantidade de cestas deve ser maior que zero. Validação de valor falhou.',
          );
        } else if (data.quantidade_cestas_solicitadas > rules.MAX_CESTAS) {
          errorBuilder.add(
            'quantidade_cestas_solicitadas',
        ...
        const numeroFamilia =
          data.numero_pessoas_familia ?? entity.numero_pessoas_familia;

        if (numeroFamilia) {
          const quantidadeRecomendada =
            this.calcularQuantidadeRecomendada(numeroFamilia);

          if (data.quantidade_cestas_solicitadas > quantidadeRecomendada) {
            const justificativa =
              data.justificativa_quantidade ?? entity.justificativa_quantidade;
      ...
      if (data.numero_pessoas_familia !== undefined) {
        if (data.numero_pessoas_familia <= 0) {
          errorBuilder.add(
            'numero_pessoas_familia',
            'Número de pessoas na família deve ser maior que zero. Validação de valor falhou.',
          );
        } else if (data.numero_pessoas_familia > rules.MAX_PESSOAS_FAMILIA) {
          errorBuilder.add(
            'numero_pessoas_familia',
      ...
      if (
        data.periodo_concessao !== undefined &&
        !data.periodo_concessao?.trim()
      ) {
        errorBuilder.add(
          'periodo_concessao',
          'Campo periodo_concessao não pode estar vazio quando fornecido. Validação de conteúdo falhou.',
        );
      }

      // Validação de origem do atendimento (se fornecida)
      if (
        data.origem_atendimento !== undefined &&
        !data.origem_atendimento?.trim()
      ) {
        errorBuilder.add(
          'origem_atendimento',
          'Campo origem_atendimento não pode estar vazio quando fornecido. Validação de conteúdo falhou.',
        );
      }
      ...
      // Validação de técnico responsável (se fornecido)
      if (
        data.tecnico_responsavel !== undefined &&
        data.tecnico_responsavel &&
        data.tecnico_responsavel.trim().length === 0
      ) {
        errorBuilder.add(
      ...
      // Validação de unidade solicitante (se fornecida)
      if (
        data.unidade_solicitante !== undefined &&
        data.unidade_solicitante &&
        data.unidade_solicitante.trim().length === 0
      ) {
        errorBuilder.add(
      ...
      this.logger.error(
        'Erro inesperado durante validação de atualização de cesta básica',
        {
          error: error.message,
          stack: error.stack,
          entity_id: entity.id,
