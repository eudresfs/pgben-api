...
export class CampoDinamicoService {
   ...
   */
  async create(tipoBeneficioId: string, createCampoDinamicoDto: CreateCampoDinamicoDto) {
    // Verificar se o tipo de benefício existe
    const tipoBeneficio = await this.tipoBeneficioRepository.findOne({
      where: { id: tipoBeneficioId }
    });
    ...
    if (campoExistente) {
      throw new ConflictException(`Já existe um campo com o nome '${createCampoDinamicoDto.nome}' para este tipo de benefício`);
    }
   ...
   */
  async update(id: string, updateCampoDinamicoDto: Partial<CreateCampoDinamicoDto>) {
    ...
    if (updateCampoDinamicoDto.nome && updateCampoDinamicoDto.nome !== campoDinamico.nome) {
      const campoExistente = await this.campoDinamicoRepository.findOne({
        where: { 
      ...
      if (campoExistente) {
        throw new ConflictException(`Já existe um campo com o nome '${updateCampoDinamicoDto.nome}' para este tipo de benefício`);
      }
    }
