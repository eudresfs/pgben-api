...
export class ValidacaoBeneficioService {
  ...
  async validarCriacaoSolicitacao(
    ...
    if (!beneficiario_id || !tipo_beneficio_id) {
      throw new BadRequestException(
        'Beneficiário e tipo de benefício são obrigatórios',
      );
    }
    ...
    if (determinacao_judicial_flag) {
      ...
      if (!determinacao_judicial_id) {
        throw new BadRequestException(
          'Determinação judicial é obrigatória para solicitações com flag determinacao_judicial_flag',
        );
      }
      ...
      await this.validarQuantidadeParcelas(
        tipo_beneficio_id,
        quantidade_parcelas,
        true,
      );
    ...
    await this.validarDuplicidadeSolicitacao(
      beneficiario_id,
      tipo_beneficio_id,
    );
   ...
   */
  private async validarDuplicidadeSolicitacao(
   ...
   */
  private async validarDuplicidadeConcessao(
    beneficiarioId: string,
    tipoBeneficioId: string,
  ): Promise<void> {
    ...
    const concessoes = await this.concessaoRepository
      ...
      .andWhere('concessao.status != :encerradaStatus', {
        encerradaStatus: StatusConcessao.CESSADO,
      })
      .getMany();
  ...
  private async validarQuantidadeParcelas(
    ...
    const quantidadeMaximaParcelas =
      tipoBeneficio.criterios_elegibilidade?.quantidade_maxima_parcelas || 12;
    ...
    if (periodicidade === 'unico' && quantidadeParcelas !== 1) {
      throw new BadRequestException(
        'Benefícios de periodicidade única devem ter exatamente 1 parcela',
      );
    }
    ...
    if (periodicidade !== 'unico') {
      if (
        ...
        quantidadeParcelas > quantidadeMaximaParcelas
      ) {
        throw new BadRequestException(
   ...
   */
  private async validarDocumentoJudicial(
    determinacaoJudicialId: string,
  ): Promise<void> {
    // Importar DeterminacaoJudicial dinamicamente para evitar dependência circular
    const { DeterminacaoJudicial } = await import(
      '../../../entities/determinacao-judicial.entity'
    );
    const determinacaoRepo =
      this.solicitacaoRepository.manager.getRepository(DeterminacaoJudicial);
    ...
    if (!determinacao) {
      throw new BadRequestException(
        `Determinação judicial com ID ${determinacaoJudicialId} não encontrada`,
      );
    }

    if (!determinacao.ativo) {
      throw new BadRequestException(
        'A determinação judicial informada não está ativa',
      );
    }

    if (!determinacao.documento_url) {
      throw new BadRequestException(
        'A determinação judicial deve possuir documento anexado',
      );
    }

    this.logger.debug(
      `Determinação judicial ${determinacaoJudicialId} validada com sucesso`,
    );
  }
}
