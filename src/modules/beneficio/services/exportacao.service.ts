...
export class ExportacaoService {
    ...
    try {
      ...
      dados.forEach(item => {
        Object.keys(item).forEach(key => {
          if (!['id', 'cidadao_id', 'tipo_beneficio', 'status', 'data_solicitacao', 'data_atualizacao'].includes(key)) {
      ...
      const cabecalhoCompleto = [...cabecalhosBasicos, ...Array.from(camposDinamicos)];
      ...
      dados.forEach(item => {
        const linha = [
          ...
          this.escaparCampoCSV(item.data_atualizacao || '')
        ];
   ...
   */
  private async transformarDadosParaCSV(solicitacoes: SolicitacaoBeneficio[]): Promise<any[]> {
    return solicitacoes.map(solicitacao => {
      // Dados básicos
      const dadosBasicos = {
        id: solicitacao.id,
        cidadao_id: solicitacao.cidadao_id,
        ...
        data_atualizacao: this.formatarData(solicitacao.updated_at)
      };
