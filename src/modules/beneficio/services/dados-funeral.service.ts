  ...
  protected async validateCreateData(
    ...
    try {
      ...
      if (!data.solicitacao_id?.trim()) {
        errorBuilder.add(
          'solicitacao_id',
          BENEFICIO_TECH_MESSAGES.FUNERAL.SOLICITACAO_ID_REQUIRED,
        );
      }
      ...
      if (!data.nome_completo_falecido?.trim()) {
        errorBuilder.add(
          ...
          BENEFICIO_TECH_MESSAGES.FUNERAL.NOME_FALECIDO_REQUIRED,
        );
      ...
      if (!data.data_obito) {
        errorBuilder.add(
          'data_obito',
          BENEFICIO_TECH_MESSAGES.FUNERAL.DATA_OBITO_REQUIRED,
        );
      }
      ...
      if (data.data_obito) {
        ...
        if (isNaN(dataObito.getTime())) {
          ...
          if (diasAposObito > rules.MAX_DIAS_APOS_OBITO) {
            errorBuilder.add(
              'data_obito',
              BENEFICIO_TECH_MESSAGES.FUNERAL.DATA_OBITO_LIMITE_EXCEDIDO,
            );
          }
          ...
          if (dataObito > hoje) {
            errorBuilder.add(
              'data_obito',
              BENEFICIO_TECH_MESSAGES.FUNERAL.DATA_OBITO_FUTURA,
            );
          }
        }
      }
