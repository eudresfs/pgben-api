...
/**
 * Repositório customizado para DadosAtaude
 * Extende o repositório base do TypeORM com métodos específicos
 */
@Injectable()
export class DadosAtaudeRepository extends Repository<DadosAtaude> {
  constructor(private dataSource: DataSource) {
    super(DadosAtaude, dataSource.createEntityManager());
  }

  /**
   * Buscar dados de ataude por solicitação com relacionamentos
   */
  async findBySolicitacaoWithRelations(
    solicitacaoId: string,
  ): Promise<DadosAtaude | null> {
    return this.findOne({
   ...
   */
  async findByTipoUrna(tipoUrna: TipoUrnaEnum): Promise<DadosAtaude[]> {
    return this.createQueryBuilder('dados')
      .leftJoinAndSelect('dados.solicitacao', 'solicitacao')
      .leftJoinAndSelect('solicitacao.beneficiario', 'cidadao')
   ...
   */
  async findByLocalObito(localObito: string): Promise<DadosAtaude[]> {
    return this.createQueryBuilder('dados')
      .leftJoinAndSelect('dados.solicitacao', 'solicitacao')
      .leftJoinAndSelect('solicitacao.beneficiario', 'cidadao')
  ...
  async findWithFilters(filters: {
    ...
    page?: number;
    limit?: number;
  }): Promise<{ data: DadosAtaude[]; total: number }> {
    const query = this.createQueryBuilder('dados')
      .leftJoinAndSelect('dados.solicitacao', 'solicitacao')
      .leftJoinAndSelect('solicitacao.beneficiario', 'cidadao');
