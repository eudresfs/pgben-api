import { Controller, Post, Body, Get, Param, UseGuards, Query, Patch, NotFoundException, BadRequestException } from '@nestjs/common';
...
export class SolicitacaoBeneficioController {
  ...
  async create(@Body() createSolicitacaoBeneficioDto: CreateSolicitacaoBeneficioDto) {
    ...
    if (!tipoBeneficio) {
      throw new Error(`Tipo de benefício com ID ${createSolicitacaoBeneficioDto.tipo_beneficio_id} não encontrado`);
    }

    // Processar e validar dados dinâmicos
    const dadosProcessados = await this.dadosDinamicosService.processarDadosDinamicos(
    ...
    // Salvar solicitação
    const solicitacaoSalva = await this.solicitacaoRepository.save(novaSolicitacao);
  ...
  @ApiResponse({ status: 200, description: 'Solicitação retornada com sucesso' })
   ...
   */
  private validarTransicaoStatus(statusAtual: string, novoStatus: string): void {
