...
import { IsEnum, IsNotEmpty, IsOptional, IsString, IsUUID, ValidateIf, IsDate } from 'class-validator';
...
export class CreatePapelCidadaoDto {
  ...
  @IsString({ message: 'Documento de representação é obrigatório para representantes legais' })
  @IsNotEmpty({ message: 'Documento de representação é obrigatório para representantes legais' })
  'metadados.documento_representacao'?: string;

  @ValidateIf(o => o.tipo_papel === TipoPapel.REPRESENTANTE_LEGAL)
  @IsDate({ message: 'Data de validade da representação deve ser uma data válida' })
  @Type(() => Date)
  @IsNotEmpty({ message: 'Data de validade da representação é obrigatória para representantes legais' })
