...
export class PagamentoCacheInvalidationService {
  ...
  private setupEventListeners(): void {
    ...
    this.eventEmitter.on('pagamento.created', (event: PagamentoCacheInvalidationEvent) => {
      this.handlePagamentoCreated(event);
    });

    // Listener para atualização de pagamentos
    this.eventEmitter.on('pagamento.updated', (event: PagamentoCacheInvalidationEvent) => {
      this.handlePagamentoUpdated(event);
    });

    // Listener para mudança de status
    this.eventEmitter.on('pagamento.status_changed', (event: PagamentoCacheInvalidationEvent) => {
      this.handleStatusChanged(event);
    });

    // Listener para cancelamento
    this.eventEmitter.on('pagamento.cancelled', (event: PagamentoCacheInvalidationEvent) => {
      this.handlePagamentoCancelled(event);
    });

    // Listener para exclusão
    this.eventEmitter.on('pagamento.deleted', (event: PagamentoCacheInvalidationEvent) => {
   ...
   */
  private async handlePagamentoCreated(event: PagamentoCacheInvalidationEvent): Promise<void> {
    try {
      this.logger.log(`Invalidando cache após criação do pagamento ${event.pagamentoId}`);
      ...
      if (event.solicitacaoId) {
        await this.invalidatePatterns(this.CACHE_PATTERNS.RELATIONSHIPS.SOLICITACAO(event.solicitacaoId));
      }
      
      if (event.concessaoId) {
        await this.invalidatePatterns(this.CACHE_PATTERNS.RELATIONSHIPS.CONCESSAO(event.concessaoId));
      }
      
      this.logger.debug(`Cache invalidado após criação do pagamento ${event.pagamentoId}`);
    } catch (error) {
      this.logger.error(`Erro ao invalidar cache após criação: ${error.message}`);
    }
  }
   ...
   */
  private async handlePagamentoUpdated(event: PagamentoCacheInvalidationEvent): Promise<void> {
    try {
      this.logger.log(`Invalidando cache após atualização do pagamento ${event.pagamentoId}`);
      
      // Invalidar cache específico da entidade
      await this.invalidatePatterns(this.CACHE_PATTERNS.ENTITY_SPECIFIC(event.pagamentoId));
      ...
      await this.pagamentoCacheService.invalidateValidationCache(event.pagamentoId);
      
      this.logger.debug(`Cache invalidado após atualização do pagamento ${event.pagamentoId}`);
    } catch (error) {
      this.logger.error(`Erro ao invalidar cache após atualização: ${error.message}`);
    }
  }
   ...
   */
  private async handleStatusChanged(event: PagamentoCacheInvalidationEvent): Promise<void> {
    try {
      this.logger.log(
      ...
      await this.invalidateStatusTransitionCache(event.oldStatus, event.newStatus);
      
      this.logger.debug(
        `Cache invalidado após mudança de status do pagamento ${event.pagamentoId}`
      );
    } catch (error) {
      this.logger.error(`Erro ao invalidar cache após mudança de status: ${error.message}`);
    }
  }
   ...
   */
  private async handlePagamentoCancelled(event: PagamentoCacheInvalidationEvent): Promise<void> {
    try {
      this.logger.log(`Invalidando cache após cancelamento do pagamento ${event.pagamentoId}`);
      ...
      this.logger.debug(`Cache invalidado após cancelamento do pagamento ${event.pagamentoId}`);
    } catch (error) {
      this.logger.error(`Erro ao invalidar cache após cancelamento: ${error.message}`);
    }
  }
   ...
   */
  private async handlePagamentoDeleted(event: PagamentoCacheInvalidationEvent): Promise<void> {
    try {
      this.logger.log(`Invalidando cache após exclusão do pagamento ${event.pagamentoId}`);
      ...
      this.logger.debug(`Cache invalidado após exclusão do pagamento ${event.pagamentoId}`);
    } catch (error) {
      this.logger.error(`Erro ao invalidar cache após exclusão: ${error.message}`);
    }
  }
  ...
  private async invalidateStatusTransitionCache(
    ...
    try {
      ...
      this.logger.warn(`Erro ao invalidar cache de transição de status: ${error.message}`);
    }
  }
  ...
  async invalidatePagamentoCache(pagamentoId: string): Promise<void> {
    await this.invalidatePatterns(this.CACHE_PATTERNS.ENTITY_SPECIFIC(pagamentoId));
  }
  ...
  async invalidateSolicitacaoCache(solicitacaoId: string): Promise<void> {
    await this.invalidatePatterns(this.CACHE_PATTERNS.RELATIONSHIPS.SOLICITACAO(solicitacaoId));
  }
  ...
  async invalidateConcessaoCache(concessaoId: string): Promise<void> {
    await this.invalidatePatterns(this.CACHE_PATTERNS.RELATIONSHIPS.CONCESSAO(concessaoId));
  }
