...
export class PagamentoCalculatorService implements IPagamentoCalculatorService {
  ...
  private readonly estrategias = new Map<
    string,
    IBeneficioCalculatorStrategy
  >();
  ...
  registrarEstrategia(strategy: IBeneficioCalculatorStrategy): void {
    ...
    this.logger.log(
      `Estratégia registrada para tipo: ${strategy.tipoBeneficio}`,
    );
  }
   ...
   */
  async calcularPagamento(
    dados: DadosPagamento,
  ): Promise<ResultadoCalculoPagamento> {
    ...
    try {
      this.logger.log(
        `Calculando pagamento para benefício: ${dados.tipoBeneficio}`,
      );
  ...
  private validarResultado(resultado: ResultadoCalculoPagamento): void {
    ...
    if (resultado.dataVencimento <= resultado.dataLiberacao) {
      throw new BadRequestException(
        'Data de vencimento deve ser posterior à data de liberação',
      );
    }
  }
