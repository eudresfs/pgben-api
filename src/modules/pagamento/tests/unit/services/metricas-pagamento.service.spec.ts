...
describe('MetricasPagamentoService', () => {
  ...
  beforeEach(async () => {
    ...
    comprovanteRepository = module.get<Repository<ComprovantePagamento>>(getRepositoryToken(ComprovantePagamento));
    confirmacaoRepository = module.get<Repository<ConfirmacaoRecebimento>>(getRepositoryToken(ConfirmacaoRecebimento));
  });
    ...
    it('deve retornar métricas de tempo real sobre pagamentos', async () => {
      ...
      const mockMetricas = {
        ...
        pagamentosMaisRecentes: [
          { id: 'pagamento-id-1', valor: 500, status: StatusPagamentoEnum.PAGO, dataAtualizacao: new Date() },
          { id: 'pagamento-id-2', valor: 300, status: StatusPagamentoEnum.LIBERADO, dataAtualizacao: new Date() }
        ],
  ...
  describe('obterDashboard', () => {
    ...
    it('deve aplicar filtros a todos os métodos quando especificados', async () => {
      ...
      // Mock dos métodos do serviço
      jest.spyOn(service, 'obterMetricasVolume').mockResolvedValueOnce({} as any);
      jest.spyOn(service, 'obterMetricasEficiencia').mockResolvedValueOnce({} as any);
      jest.spyOn(service, 'obterMetricasTempoReais').mockResolvedValueOnce({} as any);
