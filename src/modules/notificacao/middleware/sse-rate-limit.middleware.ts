...
export class SseRateLimitMiddleware implements NestMiddleware {
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      const isRateLimitingAvailable = this.gracefulDegradationService.isFeatureAvailable('rate_limiting');
      
      if (!isRateLimitingAvailable) {
        this.loggingService.logConnection(LogLevel.WARN, 'Rate limiting indisponível - permitindo request', {
      ...
      if (!rateLimitResult) {
        ...
        this.loggingService.logError(new Error('Rate limit result is undefined'), {
          userId: userId ? parseInt(userId, 10) : undefined,
  ...
  private handleRateLimitExceeded(
    ...
    res.status(429).json({
      ...
      retryAfter: rateLimitResult.retryAfter,
      remainingPoints: rateLimitResult.info.remainingPoints,
      resetTime: new Date(Date.now() + rateLimitResult.info.msBeforeNext).toISOString(),
    });
  }
