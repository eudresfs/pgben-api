...
export class SseRedisService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    const redisDisabled = this.configService.get<boolean>(
      'DISABLE_REDIS',
      false,
    );
    if (redisDisabled) {
    ...
    try {
      ...
      this.logger.warn(
        `Erro ao configurar Redis: ${error.message}. SSE funcionará apenas localmente.`,
      );
    }
  }
  ...
  async publishToUser(
    ...
    try {
      ...
      if (!this.publisher) {
        this.logger.debug(
          'Redis não disponível - ignorando publicação para usuário',
        );
        return;
      }
  ...
  async publishNotification(notification: SseNotification): Promise<void> {
    try {
      ...
      this.logger.debug(
        `📤 Notificação publicada para usuário ${notification.userId}`,
      );
    } catch (error) {
  ...
  async publishBroadcast(
    ...
    try {
      ...
      if (!this.publisher) {
        this.logger.debug(
          'Redis não disponível - ignorando publicação broadcast',
        );
        return;
      }
  ...
  async subscribeToUser(
    userId: string,
    callback: (notification: SseNotification) => void,
  ): Promise<void> {
    try {
      // Verificar se Redis está disponível
      if (!this.subscriber) {
        this.logger.debug(
          'Redis não disponível - ignorando subscrição de usuário',
        );
        return;
      }
  ...
  async subscribeToBroadcast(
    ...
    try {
      ...
      if (!this.subscriber) {
        this.logger.debug(
          'Redis não disponível - ignorando subscrição broadcast',
        );
        return;
      }
      ...
      this.subscriber.on('message', (receivedChannel, message) => {
        if (receivedChannel === channel) {
          try {
            ...
            this.logger.error(
              '❌ Erro ao processar notificação broadcast:',
              error,
            );
          }
        }
  ...
  async unsubscribeFromUser(userId: string): Promise<void> {
    try {
      ...
      if (!this.subscriber) {
        this.logger.debug(
          'Redis não disponível - ignorando remoção de subscrição',
        );
        return;
      }
  ...
  async storeConnection(connection: SseConnection): Promise<void> {
    try {
      ...
      if (!this.storage) {
        this.logger.debug(
          'Redis não disponível - ignorando armazenamento de conexão',
        );
        return;
      }
      ...
      this.logger.debug(
        `💾 Conexão ${connection.connectionId} armazenada para usuário ${connection.userId}`,
      );
    } catch (error) {
  ...
  async removeConnection(connectionId: string, userId: string): Promise<void> {
    try {
      ...
      if (!this.storage) {
        this.logger.debug(
          'Redis não disponível - ignorando remoção de conexão',
        );
        return;
      }
  ...
  async getUserConnections(userId: string): Promise<SseConnection[]> {
    try {
      ...
      if (!this.storage) {
        this.logger.debug(
          'Redis não disponível - retornando lista vazia de conexões',
        );
        return [];
      }
      ...
      const connectionKeys = connectionIds.map(
        (id) => `${this.CONNECTION_PREFIX}:${id}`,
      );
      ...
      for (let i = 0; i < connectionData.length; i++) {
        if (connectionData[i]) {
          try {
            ...
            this.logger.warn(
              `⚠️ Erro ao parsear conexão ${connectionIds[i]}:`,
              error,
            );
      ...
      this.logger.error(
        `❌ Erro ao buscar conexões do usuário ${userId}:`,
        error,
      );
      return [];
    }
  ...
  async updateHeartbeat(connectionId: string): Promise<void> {
    try {
      ...
      if (!this.storage) {
        this.logger.debug(
          'Redis não disponível - ignorando atualização de heartbeat',
        );
        return;
      }
      ...
      this.logger.error(
        `❌ Erro ao atualizar heartbeat ${connectionId}:`,
        error,
      );
    }
  }
        ...
        try {
          ...
          if (connectionData) {
            ...
            if (now.getTime() - lastHeartbeat.getTime() > expireThreshold) {
              const connectionId = key.replace(
                `${this.CONNECTION_PREFIX}:`,
                '',
              );
  ...
  async getGlobalStats(): Promise<{
    ...
    try {
      ...
      if (!this.storage) {
        this.logger.debug(
          'Redis não disponível - retornando estatísticas vazias',
        );
        return {
          totalConnections: 0,
          connectedUsers: 0,
