...
export class SseEventStoreService {
  ...
  private async initializeRedis() {
    try {
      ...
      this.logger.error(
        '❌ Erro ao inicializar Redis no SseEventStoreService:',
        error,
      );
      throw error;
    }
  ...
  async storeEvent(
    ...
    try {
      ...
      await this.redis.setex(eventKey, ttl, JSON.stringify(storedEvent));
  ...
  async replayEvents(
    ...
    try {
      ...
      const response: EventReplayResponse = {
        ...
        hasMore,
        lastEventId:
          events.length > 0 ? events[events.length - 1].eventId : undefined,
        timestamp: new Date(),
      };
  ...
  async markEventAsDelivered(eventId: string): Promise<void> {
    try {
      ...
      if (event) {
        ...
        if (ttl > 0) {
          await this.redis.setex(eventKey, ttl, JSON.stringify(event));
        }
      }
    } catch (error) {
      this.logger.error(
        `❌ Erro ao marcar evento ${eventId} como entregue:`,
        error,
      );
    }
  }
  ...
  async getLastEventId(userId: string): Promise<string | null> {
    try {
      ...
      this.logger.error(
        `❌ Erro ao obter último evento do usuário ${userId}:`,
        error,
      );
      return null;
    }
  ...
  async cleanupExpiredEvents(): Promise<number> {
    try {
      ...
      if (cleanedCount > 0) {
        this.logger.log(
          `🧹 Limpeza automática: ${cleanedCount} eventos expirados removidos`,
        );
      }
  ...
  async getStorageStats(): Promise<{
    ...
    try {
      ...
      this.logger.error(
        '❌ Erro ao obter estatísticas de armazenamento:',
        error,
      );
      return {
        totalEvents: 0,
  ...
  private async findSequenceByTimestamp(
    userId: string,
    since: Date,
  ): Promise<number> {
  ...
  private async limitUserEvents(userId: string): Promise<void> {
    ...
    if (count > this.MAX_EVENTS_PER_USER) {
      ...
      const oldEventIds = await this.redis.zrange(
        userEventsKey,
        0,
        toRemove - 1,
      );
