...
export class AblyAuthService {
   ...
   */
  async generateAuthToken(config: IAblyAuthConfig): Promise<IAblyOperationResult<IAblyTokenDetails>> {
    ...
    try {
      ...
      if (cachedToken && this.isTokenValid(cachedToken)) {
        this.logger.debug(`Token encontrado no cache para usuário ${config.userId}`);
        return {
          success: true,
          data: cachedToken,
          timestamp: new Date(),
      ...
      this.logger.debug(`Token gerado com sucesso para usuário ${config.userId}`);
      ...
      this.logger.error(`Erro ao gerar token para usuário ${config.userId}:`, error);
   ...
   */
  async generateAdminToken(userId: string, expiresIn?: number): Promise<IAblyOperationResult<IAblyTokenDetails>> {
   ...
   */
  async generateUserToken(userId: string, expiresIn?: number): Promise<IAblyOperationResult<IAblyTokenDetails>> {
    const config: IAblyAuthConfig = {
      userId,
      isAdmin: false,
      expiresIn
   ...
   */
  async revokeToken(token: string, userId: string): Promise<IAblyOperationResult> {
    try {
      // Remove do cache
      const cacheKeys = Array.from(this.tokenCache.keys()).filter(key => key.includes(userId));
  ...
  async hasChannelPermission(
    ...
    try {
      const fullChannelName = this.ablyConfig.getChannelName(channelName, userId);
  ...
  private getCacheKey(config: IAblyAuthConfig): string {
    ...
    const hash = crypto.createHash('md5').update(`${config.userId}-${config.isAdmin}-${capabilities}`).digest('hex');
  ...
  cleanExpiredTokens(): void {
    ...
    this.logger.debug(`Cache de tokens limpo. Tokens ativos: ${this.tokenCache.size}`);
  }
