...
import { SseRateLimiterService, RateLimitMetrics } from '../services/sse-rate-limiter.service';
import { JwtAuthGuard } from '../../../auth/guards/jwt-auth.guard';
import { RolesGuard } from '../../../auth/guards/roles.guard';
import { Roles } from '../../../auth/decorators/role.decorator';
import { ROLES } from '../../../shared/constants/roles.constants';
import { SseRateLimitGuard, SseRateLimitAdmin } from '../guards/sse-rate-limit.guard';
import {
  ApiTags,
  ApiOperation,
  ApiResponse,
...
export class SseRateLimitController {
  ...
  @ApiResponse({ 
    ...
    schema: {
      type: 'object',
      properties: {
        totalRequests: { type: 'number', description: 'Total de requisições processadas' },
        blockedRequests: { type: 'number', description: 'Requisições bloqueadas' },
  ...
  async getMetrics(
    ...
    this.logger.log(`Obtendo métricas de rate limiting para período de ${range}s`);
  ...
  @ApiOperation({ summary: 'Resetar rate limit para um identificador específico' })
  ...
  @ApiBody({
    ...
    schema: {
      type: 'object',
      properties: {
        identifier: { type: 'string', description: 'Identificador (IP, usuário, etc.)' },
        profile: { 
          type: 'string', 
  ...
  async resetRateLimit(@Body() resetDto: ResetRateLimitDto): Promise<{ success: boolean }> {
    this.logger.log(
  ...
  @ApiResponse({ status: 201, description: 'IP adicionado à whitelist com sucesso' })
  @ApiResponse({ status: 429, description: 'Rate limit excedido' })
  @ApiBody({
  ...
  async addToWhitelist(@Body() whitelistDto: WhitelistDto): Promise<{ success: boolean }> {
  ...
  @ApiResponse({ status: 200, description: 'IP removido da whitelist com sucesso' })
  @ApiResponse({ status: 429, description: 'Rate limit excedido' })
  @ApiParam({ name: 'ip', description: 'Endereço IP para remover' })
  async removeFromWhitelist(@Param('ip') ip: string): Promise<{ success: boolean }> {
  ...
  @Roles(ROLES.ADMIN)
  @SseRateLimitAdmin()
  @ApiOperation({ summary: 'Verificar status de rate limit para um identificador' })
  @ApiResponse({ 
    status: 200, 
