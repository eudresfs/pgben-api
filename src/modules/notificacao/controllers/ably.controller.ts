...
export class AblyController {
  ...
  @ApiOperation({ 
    ...
    description: 'Gera um token JWT para autenticação do cliente no Ably com permissões baseadas no perfil do usuário'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  async generateAuthToken(@Request() req: any) {
    try {
      ...
      this.logger.debug(`Gerando token Ably para usuário ${userId}, admin: ${isAdmin}`);
      
      const result = await this.orchestratorService.generateClientToken(userId, isAdmin);
  ...
  @ApiOperation({ 
    ...
    description: 'Envia uma notificação para um usuário específico usando a melhor estratégia disponível (Ably ou SSE)'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  async sendNotification(
    ...
    try {
      ...
      const notification: IAblyNotificationData = {
        ...
        priority: createNotificationDto.priority as NotificationPriority || 'normal',
      ...
      this.logger.debug(`Enviando notificação ${notification.id} para usuário ${createNotificationDto.userId}`);
      ...
      const result = await this.orchestratorService.publishNotification(
        ...
        notification,
        {
          ...
          priority: createNotificationDto.priority === NotificationPriority.URGENT ? 'high' : 
                   createNotificationDto.priority === NotificationPriority.HIGH ? 'high' :
                   createNotificationDto.priority === NotificationPriority.LOW ? 'low' : 'normal'
        }
      );
  ...
  @ApiOperation({ 
    ...
    description: 'Envia uma notificação em broadcast para grupos de usuários (requer permissões administrativas)'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  async sendBroadcast(
    ...
    try {
      ...
      const notification: IAblyNotificationData = {
        ...
        priority: broadcastDto.priority as NotificationPriority || 'normal',
      ...
      this.logger.debug(`Enviando broadcast ${notification.id} para target:`, broadcastDto.target);
  ...
  @ApiOperation({ 
    ...
    description: 'Retorna estatísticas detalhadas do Ably e do orquestrador de notificações (requer permissões administrativas)'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  @ApiOperation({ 
    ...
    description: 'Endpoint de health check para verificar o status do Ably e dos sistemas de notificação'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  @ApiOperation({ 
    ...
    description: 'Retorna lista de canais ativos com suas estatísticas (requer permissões administrativas)'
  })
  @ApiQuery({ name: 'type', required: false, description: 'Filtrar por tipo de canal' })
  @ApiResponse({ 
    status: 200, 
  ...
  @ApiOperation({ 
    ...
    description: 'Força a mudança do método de entrega de notificações (requer permissões administrativas)'
  })
  @ApiResponse({ 
    status: 200, 
  ...
  async switchDeliveryMethod(
    ...
    try {
      ...
      this.logger.log(`Método de entrega alterado para ${method} por ${req.user.email}`);
