...
export class MetricasService {
  ...
  async criarMetrica(
    ...
    if (metricaExistente) {
      throw new BadRequestException(`Já existe uma métrica com o código '${dto.codigo}'`);
    }
   ...
   * @returns Métrica encontrada ou null
   */
  async buscarMetricaPorCodigo(codigo: string): Promise<MetricaDefinicao | null> {
   ...
   */
  async listarMetricas(filtros: FiltroMetricasDto): Promise<{ items: MetricaDefinicao[], total: number }> {
  ...
  async criarConfiguracao(
    ...
    if (!metrica) {
      throw new NotFoundException(`Métrica com ID ${dto.metrica_id} não encontrada`);
    }
   ...
   */
  async buscarConfiguracaoPorMetrica(metricaId: string): Promise<MetricaConfiguracao> {
    const config = await this.metricaConfiguracaoRepository.findOne({
    ...
    if (!config) {
      throw new NotFoundException(`Configuração para métrica ${metricaId} não encontrada`);
    }
   ...
   */
  private async validarDadosMetrica(metrica: CriarMetricaDefinicaoDto | MetricaDefinicao): Promise<void> {
    ...
    switch (metrica.tipo) {
      ...
      default:
        throw new BadRequestException(`Tipo de métrica não suportado: ${metrica.tipo}`);
    }
  }
   ...
   */
  private async validarDadosConfiguracao(config: CriarMetricaConfiguracaoDto | MetricaConfiguracao): Promise<void> {
    ...
    if (config.estrategia_amostragem !== 'completa' && !config.tamanho_amostra) {
      throw new BadRequestException(
    ...
    if (config.alertas && config.alertas.length > 0) {
      for (const alerta of config.alertas) {
        if (!alerta.tipo) {
          throw new BadRequestException('Cada alerta deve ter um tipo definido');
        }
        
        if (alerta.valor === undefined || alerta.valor === null) {
          throw new BadRequestException(`Alerta do tipo '${alerta.tipo}' requer um valor de referência`);
        }
        
        if (!alerta.severidade) {
          throw new BadRequestException(`Alerta do tipo '${alerta.tipo}' requer uma severidade definida`);
        }
      }
    }
  }
