...
const getSecurityConfig = (configService: ConfigService): SecurityConfig => {
  ...
  if (isProduction && (!cookieSecret || !sessionSecret)) {
    throw new Error('COOKIE_SECRET e SESSION_SECRET são obrigatórios em produção');
  }
  ...
  return {
    ...
    allowedOrigins: configService.get<string>('ALLOWED_ORIGINS', 'http://localhost:3001')
      .split(',')
...
const generateSecureSecret = (): string => {
  ...
  securityLogger.warn(`Secret gerado automaticamente para desenvolvimento: ${secret.substring(0, 8)}...`);
  return secret;
};
...
const createSecurityMiddleware = (config: SecurityConfig) => {
  return (req: any, res: any, next: any) => {
    ...
    if (req.headers['x-forwarded-for'] && req.headers['x-forwarded-for'].split(',').length > 3) {
      securityLogger.warn(`Possível proxy chain suspeito: ${req.headers['x-forwarded-for']}`);
    }
 ...
 */
export const applySecurity = async (app: INestApplication, configService: ConfigService) => {
  try {
    ...
    try {
      ...
      securityLogger.log('   - Headers de segurança (Helmet) aplicados com sucesso');
    } catch (error) {
      securityLogger.warn('   - Erro ao aplicar Helmet com configuração completa:', error.message);
    ...
    securityLogger.log(`   - Modo: ${securityConfig.isProduction ? 'Produção' : 'Desenvolvimento'}`);
    ...
    securityLogger.log(`   - Origins permitidas: ${securityConfig.allowedOrigins.join(', ')}`);
    
  } catch (error) {
    securityLogger.error('❌ Erro ao aplicar configurações de segurança:', error);
    throw error;
  }
};
...
export const CookieUtils = {
  ...
  setSecure: (
    ...
    const cookieOptions = CookieUtils.createSecureOptions(configService, options);
...
export const createRateLimitHeaders = (limit: number, windowMs: number) => {
  return (req: any, res: any, next: any) => {
    res.setHeader(SECURITY_CONSTANTS.HEADERS.RATE_LIMIT.LIMIT, limit.toString());
    res.setHeader(SECURITY_CONSTANTS.HEADERS.RATE_LIMIT.REMAINING, (limit - (req.rateLimit?.count || 0)).toString());
    res.setHeader(SECURITY_CONSTANTS.HEADERS.RATE_LIMIT.RESET, new Date(Date.now() + windowMs).toISOString());
    next();
  };
};
