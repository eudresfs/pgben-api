...
export const createSseConfig = (configService: ConfigService): SseConfig => {
  return {
    ...
    heartbeatInterval: configService.get<number>('REDIS_SSE_HEARTBEAT_INTERVAL', 30000),
    cleanupInterval: configService.get<number>('REDIS_SSE_CLEANUP_INTERVAL', 60000),
    maxConnectionsPerUser: configService.get<number>('SSE_MAX_CONNECTIONS_PER_USER', 5),
    reconnectionTimeout: configService.get<number>('SSE_RECONNECTION_TIMEOUT', 5000),
    instanceId: configService.get<string>('INSTANCE_ID', `instance-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`),
    
    adaptiveHeartbeat: {
      enabled: configService.get<boolean>('SSE_ADAPTIVE_HEARTBEAT_ENABLED', true),
      baseInterval: configService.get<number>('SSE_ADAPTIVE_HEARTBEAT_BASE_INTERVAL', 30000),
      minInterval: configService.get<number>('SSE_ADAPTIVE_HEARTBEAT_MIN_INTERVAL', 10000),
      maxInterval: configService.get<number>('SSE_ADAPTIVE_HEARTBEAT_MAX_INTERVAL', 120000),
      backoffFactor: configService.get<number>('SSE_ADAPTIVE_HEARTBEAT_BACKOFF_FACTOR', 1.5),
      maxMissedHeartbeats: configService.get<number>('SSE_ADAPTIVE_HEARTBEAT_MAX_MISSED', 3),
      responseTimeout: configService.get<number>('SSE_ADAPTIVE_HEARTBEAT_RESPONSE_TIMEOUT', 10000),
    },
    
    deadConnectionDetection: {
      enabled: configService.get<boolean>('SSE_DEAD_CONNECTION_DETECTION_ENABLED', true),
      checkInterval: configService.get<number>('SSE_DEAD_CONNECTION_CHECK_INTERVAL', 60000),
      timeout: configService.get<number>('SSE_DEAD_CONNECTION_TIMEOUT', 180000),
      maxPingAttempts: configService.get<number>('SSE_DEAD_CONNECTION_MAX_PING_ATTEMPTS', 3),
    },
  };
};
