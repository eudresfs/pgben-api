...
export class PerformanceMonitorMiddleware implements NestMiddleware {
  ...
  public getPerformanceStats(): {
    ...
    const totalResponseTime = this.recentMetrics.reduce((sum, m) => sum + m.duration, 0);
    const slowRequests = this.recentMetrics.filter(m => m.duration > this.slowRequestThreshold).length;
    const errorRequests = this.recentMetrics.filter(m => m.statusCode >= 400).length;
    ...
    const totalMemoryAllocated = heapUsages.reduce((sum, usage) => sum + usage, 0);
    ...
    return {
      totalRequests,
      averageResponseTime: Math.round((totalResponseTime / totalRequests) * 100) / 100,
      slowRequests,
      ...
      memoryStats: {
        ...
        totalMemoryAllocated: Math.round(totalMemoryAllocated / 1024 / 1024 * 100) / 100, // MB
      },
      topSlowEndpoints,
    };
  }
