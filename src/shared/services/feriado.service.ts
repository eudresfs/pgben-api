...
export class FeriadoService {
  ...
  async buscarFeriadosNacionais(ano: number): Promise<Set<string>> {
    ...
    try {
      ...
      this.logger.log(`Feriados carregados para o ano ${ano}: ${feriadosSet.size} feriados`);
      return feriadosSet;
    } catch (error) {
      this.logger.error(`Erro ao buscar feriados para o ano ${ano}:`, error.message);
   ...
   */
  async adicionarDiasUteis(dataInicial: Date, diasUteis: number): Promise<Date> {
    if (diasUteis === 0) {
  ...
  getEstatisticasCache(): { anosCarregados: number[], totalFeriados: number } {
    ...
    const totalFeriados = Array.from(this.feriados.values())
      .reduce((total, feriadosAno) => total + feriadosAno.size, 0);
