...
export class ValidationErrorInterceptor implements NestInterceptor {
  ...
  private enhanceErrorMessage(message: string): string {
    ...
    if (
      message.includes('must be longer than') ||
  ...
  private enhanceLengthErrorMessage(message: string): string {
    return message
      .replace(
        /must be longer than (\d+) characters?/gi,
        'deve ter mais de $1 caracteres',
      )
      .replace(
        /must be shorter than (\d+) characters?/gi,
        'deve ter menos de $1 caracteres',
      )
      .replace(
        /must contain at least (\d+) characters?/gi,
        'deve conter pelo menos $1 caracteres',
      )
      .replace(
        /must contain at most (\d+) characters?/gi,
        'deve conter no máximo $1 caracteres',
      );
  }
  ...
  private enhanceNumericErrorMessage(message: string): string {
    return message
      .replace(
        /must not be less than (\d+(?:\.\d+)?)/gi,
        'deve ser maior ou igual a $1',
      )
      .replace(
        /must not be greater than (\d+(?:\.\d+)?)/gi,
        'deve ser menor ou igual a $1',
      )
...
export class ValidationMessageHelper {
   ...
   */
  static invalidLength(fieldName: string, min?: number, max?: number): string {
    if (min && max) {
   ...
   */
  static invalidRange(fieldName: string, min?: number, max?: number): string {
