...
export class PermissionService {
   ...
   */
  async checkRolePermission(userId: string, permissionName: string): Promise<boolean> {
    try {
      // Busca a permissão pelo nome
      const permission = await this.permissionRepository.findByName(permissionName);
      if (!permission) {
   ...
   */
  async getUserPermissions(userId: string, includeInactive: boolean = false): Promise<Permission[]> {
    try {
