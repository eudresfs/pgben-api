...
export class AprovacaoInterceptor implements NestInterceptor {
  ...
  private async criarSolicitacaoAprovacao(
    ...
    const acaoCritica = await this.buscarAcaoCritica(config.acao);
    
    if (!acaoCritica) {
      throw new BadRequestException(
        `Ação crítica não encontrada para o tipo ${config.acao}`,
      );
    }
  ...
  private extrairDadosContexto(
    ...
    switch (config.acao) {
      ...
      case TipoAcaoCritica.CANCELAR_SOLICITACAO:
   ...
   */
  /**
   * Busca ação crítica pelo tipo
   */
  private async buscarAcaoCritica(tipoAcao: TipoAcaoCritica) {
    return await this.aprovacaoService['acaoCriticaRepository'].findOne({
      where: { tipo: tipoAcao }
    });
  }
