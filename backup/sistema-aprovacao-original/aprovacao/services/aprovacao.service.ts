...
export class AprovacaoService {
  ...
  private async obterConfiguracaoAprovacao(
    ...
    // Buscar ação crítica pelo campo 'tipo' para manter consistência com o interceptor
    const acaoCritica = await this.acaoCriticaRepository.findOne({
      where: { tipo: tipoAcao },
    });

    if (!acaoCritica) {
      this.logger.warn(`Ação crítica não encontrada para o tipo: ${tipoAcao}`);
      return null;
    }
    ...
    if (!configuracao) {
      this.logger.warn(`Configuração de aprovação não encontrada para ação crítica: ${acaoCritica.id}`);
    }
   ...
   async buscarSolicitacaoPendente(
     ...
     try {
       ...
       const queryBuilder = this.solicitacaoRepository
         ...
         .where('solicitacao.usuario_solicitante_id = :solicitanteId', { solicitanteId })
         .andWhere('solicitacao.status = :status', { status: StatusSolicitacaoAprovacao.PENDENTE })
         .andWhere('solicitacao.acao_critica_id = :acaoCriticaId', { acaoCriticaId: acaoCritica.id });

       if (chaveContexto) {
         queryBuilder.andWhere('solicitacao.contexto @> :contexto', {
           contexto: JSON.stringify({ chave: chaveContexto })
         });
       }
