...
export class AcaoCriticaService {
  ...
  /**
   * Busca uma ação crítica pelo tipo
   * @param tipo - Tipo da ação crítica
   * @returns Ação crítica encontrada
   */
  async findByCode(tipo: TipoAcaoCritica): Promise<AcaoCritica | null> {
   ...
   */
  async isActive(tipo: TipoAcaoCritica): Promise<boolean> {
    const acaoCritica = await this.findByCode(tipo);
