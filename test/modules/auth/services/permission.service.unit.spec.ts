...
describe('PermissionService', () => {
  ...
  describe('hasPermission', () => {
    ...
    it('deve verificar permissão direta quando não estiver em cache', async () => {
      ...
      expect(
        userPermissionRepository.findValidPermissions,
      ).toHaveBeenCalledWith(userId, mockPermission.id);
    ...
    it('deve verificar permissão de role quando não tiver permissão direta', async () => {
      ...
      expect(
        rolePermissionRepository.findPermissionsByUserRoles,
      ).toHaveBeenCalledWith(userId, mockPermission.id);
      expect(cacheManager.set).toHaveBeenCalledWith(
        expect.stringContaining(userId),
        true,
    ...
    it('deve verificar permissão composta quando não tiver permissão direta nem via role', async () => {
      ...
      expect(permissionRepository.findComposedPermissions).toHaveBeenCalledWith(
        mockPermission.id,
      );
      expect(
        userPermissionRepository.findValidPermissions,
      ).toHaveBeenCalledWith(userId, childPermission.id);
      expect(cacheManager.set).toHaveBeenCalledWith(
        expect.stringContaining(userId),
        true,
        expect.any(Number),
  ...
  describe('grantPermission', () => {
    it('deve atribuir uma nova permissão a um usuário', async () => {
      ...
      permissionRepository.findByName.mockResolvedValueOnce(mockPermission);
      userPermissionRepository.findByUserAndPermission.mockResolvedValueOnce(
        null,
      );
      ...
      expect(
        userPermissionRepository.findByUserAndPermission,
      ).toHaveBeenCalledWith(userId, mockPermission.id, scopeType, scopeId);
      expect(
        userPermissionRepository.createUserPermission,
