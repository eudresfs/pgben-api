...
import {
  UserPermission,
  ScopeType,
} from '../../src/auth/entities/user-permission.entity';
...
describe('Fluxo de Autenticação e Autorização (e2e)', () => {
  ...
  async function assignPermissionsToUser(userId: string) {
    ...
    // Buscar permissões
    const viewPermission = await permissionRepository.findOne({
      where: { name: 'usuario.visualizar' },
    });
    const editPermission = await permissionRepository.findOne({
      where: { name: 'usuario.editar' },
    });
  ...
  describe('Gerenciamento de Permissões', () => {
    it('deve listar as permissões do usuário atual', async () => {
      ...
      expect(
        response.body.some((p) => p.name === 'usuario.visualizar'),
      ).toBeTruthy();
    });
